...
const paymentColor = { "كاش": "bg-emerald-100 text-emerald-800", "آجل": "bg-orange-100 text-orange-800" };
const branchColor = {
            ...
            {invoices.map((inv) => {
              ...
              return (
                <TableRow key={inv.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <TableCell className="text-green-600">{inv.paid_value ? inv.paid_value.toLocaleString("ar-EG") : "—"}</TableCell>
                  ...
                  <TableCell>
                    {(canSaveInvoice || canDeleteInvoice) ? (
                      <div className="flex gap-1">
                        {canSaveInvoice && <Button size="icon" variant="ghost" className="h-7 w-7 text-blue-600 hover:bg-blue-50" onClick={() => onEdit(inv)}><Pencil className="w-3.5 h-3.5" /></Button>}
                        {canDeleteInvoice && <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500 hover:bg-red-50" onClick={() => onDelete(inv.id)}><Trash2 className="w-3.5 h-3.5" /></Button>}
                      </div>
