...
export default function MedicineItemsAdmin() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["medicine-items"] }); setNewName(""); },
  });
  ...
  const handleAdd = (e) => {
    ...
    createMutation.mutate({ name: newName.trim(), is_active: true });
  };
    ...
    <div className="space-y-4 max-w-md">
      <Card className="p-4">
        ...
        <form onSubmit={handleAdd} className="flex gap-2">
          ...
          <Button type="submit" className="bg-teal-600 hover:bg-teal-700 gap-1" disabled={createMutation.isPending}>
