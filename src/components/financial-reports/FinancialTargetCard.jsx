...
import { monthLabel, managementMonthForRange, buildBranchTargetsSummary, isManagementCycleRange } from "@/lib/financial-target-utils";
...
export default function FinancialTargetCard({ handovers, targets, dateFrom, dateTo }) {
  const [open, setOpen] = useState(false);
  const validCycle = isManagementCycleRange(dateFrom, dateTo);
  const month = managementMonthForRange(dateFrom, dateTo);
  const summary = useMemo(
    () => buildBranchTargetsSummary(handovers, targets, dateFrom, dateTo),
    [handovers, targets, dateFrom, dateTo]
  );
  const totalTarget = summary.reduce((s, b) => s + b.targetAmount, 0);
  const totalAchieved = summary.reduce((s, b) => s + b.achieved, 0);
  const percent = validCycle && totalTarget > 0 ? (totalAchieved / totalTarget) * 100 : 0;
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-xs mb-0.5 leading-tight text-white/80">
          {validCycle ? `تارجت المبيعات (${monthLabel(month)})` : "تارجت المبيعات"}
        </p>
        {validCycle ? (
          <>
            <p className="text-base md:text-lg font-extrabold text-white">{fmtCurrency(totalTarget)}</p>
            <p className="text-[11px] text-white/80 mt-0.5">{percent.toFixed(1)}% محقق داخل دورة 26→25</p>
          </>
        ) : (
          <p className="text-sm font-bold text-white mt-1">اختر «الدورة 26→25» لعرض مقارنة التارجت</p>
        )}
      </button>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-right">
              {validCycle ? `تارجت الفروع — ${monthLabel(month)} — دورة 26→25` : "تارجت الفروع"}
            </DialogTitle>
          </DialogHeader>
          {!validCycle ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
              مقارنة التارجت الشهري لا تُعرض على يوم أو أسبوع أو شهر ميلادي أو فترة مخصصة، لأن تارجت صيدليات دواء مرتبط بدورة 26→25. اختر «الدورة 26→25» من فلتر الفترة للحصول على نسبة صحيحة.
            </div>
          ) : (
                ...
                return (
                  <div key={b.branch} className="border rounded-lg p-3 space-y-1.5">
                    ...
                    <div className="flex justify-between flex-wrap gap-1 text-sm text-gray-500">
                      ...
                      <span>التارجت: <span className="font-semibold text-gray-700">{fmtCurrency(b.targetAmount)}</span></span>
                    </div>
                    {b.recordedDays > 0 && (
                      <p className="text-xs text-gray-400">
                        بمعدل يومي {fmtCurrency(b.dailyAvg)} على {b.recordedDays} يوم مسجل ← توقع نهاية الدورة {fmtCurrency(b.projectedTotal)}
                      </p>
                    )}
                    ...
                    <p className="text-xs text-gray-400">{b.percentOfTarget.toFixed(1)}% من تارجت الدورة</p>
                  </div>
                );
              })}
            </div>
