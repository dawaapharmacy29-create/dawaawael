...
/**
 * صفحة أرصدة موردين مستقلة تماماً لفرع واحد — تُستخدم عن طريق صفحتَي
 * SupplierBalancesShokry و SupplierBalancesShami، كل واحدة بتمرر branch الخاص بيها فقط.
 * كل فرع هنا نظام مالي منفصل بالكامل: أرصدة + كشف حساب + سجل مدفوعات، من غير أي تداخل مع الفرع التاني.
 */
export default function BranchSupplierBalances({ branch, accentColor = "text-blue-700" }) {
  ...
  const allSupplierNames = useMemo(
    () => getAllCreditSupplierNames({ invoices, debts }),
    [invoices, debts]
  );
  ...
  const supplierGroups = useMemo(() => {
    ...
    allSupplierNames.forEach((name) => {
      ...
      map[name] = {
        name,
        invoices: [...result.oldInvoices, ...result.newInvoices],
        ...result,
      };
    });
  ...
  return (
    <div dir="rtl" className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className={`text-2xl font-bold ${accentColor}`}>أرصدة الموردين — {branch}</h1>
          <p className="text-gray-500 text-sm mt-0.5">نظام مالي مستقل بالكامل عن باقي الفروع</p>
        </div>
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2">
          <Wallet className="w-5 h-5 text-red-500" />
          <div>
            <p className="text-xs text-gray-500">إجمالي الصافي المتبقي — {branch}</p>
      ...
      {/* Tab: Statement (مقفول على هذا الفرع فقط) */}
      {tab === "statement" && <SupplierStatement branch={branch} />}

      {/* Tab: Payments Log (مقفول على هذا الفرع فقط) */}
      {tab === "payments" && <PaymentsLog branch={branch} />}
                  ...
                  <Card key={group.name} className="overflow-hidden">
                    ...
                    {isExpanded && (
                      <div className="border-t">
                        ...
                        {payments.filter((p) => p.supplier_name === group.name && (!p.branch || p.branch === branch)).length > 0 && (
                          <div className="p-4 border-t bg-green-50">
                            <p className="text-xs font-semibold text-green-700 mb-2">سجل المدفوعات</p>
                            <div className="space-y-1">
                              {payments.filter((p) => p.supplier_name === group.name && (!p.branch || p.branch === branch)).map((p) => (
