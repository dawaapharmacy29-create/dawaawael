...
export default function OAuthConsent() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        if (!ctx) {
          setError("رابط التفويض غير صالح أو انتهت صلاحيته.");
          return;
        }
        ...
        setError("تعذر تحميل طلب التفويض. حاول مرة أخرى.");
      } finally {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if ([400, 403, 404, 409].includes(res.status)) {
          ...
          setReconnect(detail || "لم يعد من الممكن إكمال هذا التفويض. أعد الاتصال من التطبيق الذي طلب الربط ثم حاول مرة أخرى.");
          setSubmitting(false);
          return;
        }
        throw new Error("تعذر إكمال التفويض. حاول مرة أخرى.");
      }
      const data = await res.json();
  ...
  if (checking) {
    return (
      <AuthLayout icon={ShieldCheck} title="السماح بالوصول">
  ...
  const client = (info && info.client_name) || "التطبيق المتصل";
  const appName = (info && info.app_name) || "هذا التطبيق";

  if (decided) {
    return (
      <AuthLayout
        icon={ShieldCheck}
        title={decided === "approve" ? "تم السماح بالوصول" : "تم رفض الوصول"}
        subtitle={`يمكنك العودة إلى ${client} وإغلاق هذه النافذة.`}
      />
    );
  }
  ...
  if (reconnect) {
    return (
      <AuthLayout icon={ShieldCheck} title="مطلوب إعادة الاتصال">
  ...
  return (
    <AuthLayout
      ...
      subtitle={`${client} يطلب الوصول إلى ${appName} نيابةً عنك`}
    >
      {error && (
      ...
      <p className="text-sm font-medium text-foreground mb-2">
        {tools.length ? `سيتمكن من استخدام الأدوات التالية داخل ${appName}:` : "لم يتم طلب أي أدوات"}
      </p>
      {tools.length > 0 && (
