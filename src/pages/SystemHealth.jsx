...
export default function SystemHealth() {
  ...
  const { data: shifts = [], isLoading: shiftsLoading } = useQuery({
    ...
    queryFn: () => base44.entities.ShiftDelivery.filter({ shift_date: { $gte: recentFrom } }, "-shift_date", 3000),
    staleTime: 120000,
  });
  ...
  return (
    <div dir="rtl" className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Badge className={issueCount > 0 ? "bg-amber-100 text-amber-800 border border-amber-200" : "bg-emerald-100 text-emerald-800 border border-emerald-200"}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            ...
            {failedSync.slice(0, 20).map((r) => <div key={`sync-${r.id}`} className="rounded-lg border border-rose-100 bg-rose-50 p-3 text-sm"><b>مزامنة متعثرة:</b> {r.entity_type || r.entity || "سجل"} — {r.status}</div>)}
