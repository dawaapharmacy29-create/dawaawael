  ...
  return <div dir="rtl" className="p-4 md:p-6 space-y-5">
    ...
    {!supplierName ? <Card className="p-12 text-center text-gray-400"><Target className="w-10 h-10 mx-auto mb-2 opacity-30"/><p>اختر موردًا لتظهر الصورة المالية والتفاوضية كاملة.</p></Card> : loading ? <Card className="p-10 text-center text-gray-400">جاري تجهيز تحليل المورد...</Card> : <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric title="الرصيد الحالي علينا" value={money(currentBalance)} subtitle="المتبقي الحالي + مديونية/تسويات − دفعات عامة" icon={HandCoins} tone="rose"/>
        <Metric title="مسحوبات الفترة" value={money(summary.withdrawals)} subtitle={`${summary.invoiceCount.toLocaleString("ar-EG")} فاتورة`} icon={WalletCards} tone="teal"/>
        ...
        <Metric title="متوسط قيمة الفاتورة" value={money(summary.avgInvoice)} subtitle="للفترة المختارة" icon={Activity} tone="blue"/>
        ...
        <Metric title="المرتجعات في الفترة" value={money(summary.returns)} subtitle="مخصومة من المسحوبات الصافية" icon={FileText} tone="amber"/>
      </div>
