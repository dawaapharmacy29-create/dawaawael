...
export default function SupplierIntelligence() {
  ...
  const branchComparison = useMemo(() => BRANCHES.map((b) => {
    ...
    const periodOutstanding = rows.filter((i)=>i.payment_type === "آجل").reduce((s,i)=>s+Math.max(0,invoiceGross(i)-(Number(i.paid_value)||0)),0);
    ...
    return { branch:b, purchases:round2(purchases), periodOutstanding:round2(periodOutstanding), invoiceCount:rows.length, avgInvoice:rows.length?purchases/rows.length:0, currentBalance:round2(branchOpen+legacy-branchGeneral) };
